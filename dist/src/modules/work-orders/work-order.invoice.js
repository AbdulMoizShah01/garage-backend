...
exports.createInvoicePdf = exports.buildInvoiceCode = void 0;
...
const ASSETS_LOGO_PATH = node_path_1.default.resolve(__dirname, "../../../assets/Full logo.png");
const toNumber = (value) => value instanceof client_1.Prisma.Decimal ? value.toNumber() : Number(value ?? 0);
const currency = new Intl.NumberFormat("en-RW", {
    style: "currency",
    currency: "RWF",
});
const buildInvoiceCode = (order) => {
    const year = order.createdAt?.getFullYear() ?? new Date().getFullYear();
    return `${work_order_constants_1.INVOICE_PREFIX}/${year}-${String(order.id).padStart(4, "0")}`;
};
exports.buildInvoiceCode = buildInvoiceCode;
const drawLogo = (doc) => {
    if (node_fs_1.default.existsSync(ASSETS_LOGO_PATH)) {
        try {
...
const createInvoicePdf = (order) => new Promise((resolve, reject) => {
    ...
    drawLogo(doc);
    const invoiceCode = (0, exports.buildInvoiceCode)({
        id: order.id,
        createdAt: order.createdAt,
    });
    const invoiceDate = (0, date_fns_1.format)(new Date(), "PPP");
    doc
        .fontSize(12)
        .font("Helvetica-Bold")
    ...
    doc
        ...
        : "Walk-in Customer", 40)
        .text(order.customer?.phone ? `Phone: ${order.customer.phone}` : "", 40)
        .moveDown(0.5);
    doc
        .font("Helvetica-Bold")
        .text("Vehicle Details", 240, 130)
        .font("Helvetica")
        .text(`${order.vehicle.year} ${order.vehicle.make} ${order.vehicle.model}`, 240)
    ...
    order.lineItems.forEach((item) => {
        drawTableRow(doc, position, item.description, item.quantity, toNumber(item.unitPrice), toNumber(item.lineTotal));
        position += 20;
    });
    doc
    ...
    doc
        ...
        .text(`Parking: ${currency.format(parkingCharge)}`, 40, summaryStart + 48)
        .text(`Discount: ${currency.format(discount)}`, 40, summaryStart + 64);
    doc
        .font("Helvetica-Bold")
        .text(`VAT (${Math.round(work_order_constants_1.VAT_RATE * 100)}%): ${currency.format(taxes)}`, 320, summaryStart + 16, { align: "right" })
        .text(`Total: ${currency.format(total)}`, 320, summaryStart + 40, { align: "right" });
    if (order.assignments.length) {
        ...
        order.assignments.forEach((assignment, index) => {
            doc.text(`${index + 1}. ${assignment.worker.name}${assignment.role ? ` - ${assignment.role}` : ""}`, 40);
        });
    }
    doc
        .moveDown()
        .fontSize(10)
        .fillColor("#4a5568")
        .text("Thank you for choosing our garage. Payment is due upon receipt of this invoice.", { align: "center" });
    doc.end();
});
exports.createInvoicePdf = createInvoicePdf;
//# sourceMappingURL=work-order.invoice.js.map